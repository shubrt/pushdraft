...
import { uploadPayloadSchema } from "@pp/contracts";
...
describe("upload", () => {
  test("sends bearer auth and reuses the saved draft mapping", async () => {
    ...
    saveCredentials(statePaths, "pp_secret", "https://pp.example");
    ...
    const fetchImpl = async (
      ...
      expect(new Headers(init?.headers).get("Authorization")).toBe("Bearer pp_secret");
      ...
      return Response.json({
        ...
        title: "Private plan",
        requestId: null,
        publicUrl: "https://q43kvvtxix1x.pp.example",
        rawUrl: "https://q43kvvtxix1x.pp.example/raw",
        warnings: [],
      });
    };
    ...
    expect(readDraftState(statePaths).files[htmlFile]?.apiKeyFingerprint).toBe(
      fingerprintApiKey("pp_secret"),
    );
    expect(fs.readFileSync(statePaths.drafts, "utf8")).not.toContain("pp_secret");
  });
  ...
  test("fails before the request when no API key exists", async () => {
    ...
    try {
      ...
      expect(error).toHaveProperty("message", "Missing API key. Run: pp auth login");
    }
  });
  ...
  test("does not reuse a mapping after the API key changes", async () => {
    ...
    const htmlFile = path.join(homeDirectory, "plan.html");
    fs.writeFileSync(htmlFile, "<title>Private plan</title>");
    saveCredentials(statePaths, "first_key", "https://pp.example");
  ...
  test("reuses a mapping after verified key rotation in the same account", async () => {
    ...
    const htmlFile = path.join(homeDirectory, "plan.html");
    fs.writeFileSync(htmlFile, "<title>Private plan</title>");
    saveCredentials(statePaths, "first_key", "https://pp.example", "account_1");
      ...
      `${JSON.stringify({
        files: {
          [htmlFile]: {
            ...
            publicUrl: "https://legacy.pp.example",
            rawUrl: "https://legacy.pp.example/raw",
...
function makeTemporaryDirectory(): string {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "pp-cli-test-"));
